...
Hierarchical clustering can be performed by \Rfunction{hierarchicalPlot} to produce a dendrogram displaying the hierarchical relationship among samples (Figure 5). The vertical axis shows the dissimilarity (measured by means of the Euclidean distance) between samples: similar samples appear on the same branches. Colors correspond to groups. To avoid error while generating hierarchical plot (Figure 5) due to zero values in the dataset, a minimum value is added to the intensity data. 
...
To check the efficiency of the immunoprecipitation approach in the qPLEX-RIME method, a plot with the peptide sequence coverage of the bait protein can be illustrated. \Rfunction{coveragePlot} allows to plot peptide sequence coverage for the bait protein (Figure 7). The plot shows the location of peptides that have been identified with high confidence across the protein sequence and the corresponding percentage of the coverage. For a better evaluation of the pull down assay we can compare the observed bait protein coverage with the theoretical coverage.
...
p2 <- intensityPlot(MSnset_norm, title = "Quantile")
MSnset_norm <- normalizeScaling(MSnset_data,func=median)
p3 <- intensityPlot(MSnset_norm, title = "Scaling")
MSnset_norm <- groupScaling(MSnset_data,func=median, Grp="SampleGroup")
p4 <- intensityPlot(MSnset_norm, title = "WithinGrp Scaling")
grid.newpage()
...
The aggregation can be performed by sum, mean or median of the raw or normalized peptide intensities. The summarized intensity for a selected protein could be visualized using \Rfunction{peptideIntensityPlot}. It plots all peptides intensities alongwith summarized one across all the samples (Figure 9).
...
\begin{center}
<<pepIntensity,fig.width=6,fig.height=5,out.width='.85\\textwidth',message=FALSE,cache=TRUE>>=
...
The results of the differential protein analysis can be visualized using \Rfunction{maVolPlot} function. It plots average log2 protein intensity to log2 fold change between groups compared. This enables quick visualization (Figure 10) of significantly changing proteins between groups. \Rfunction{maVolPlot} could also be used to view differential protein results in a volcano plot (Figure 11) to compare the size of the fold change to the statistical significance level.
