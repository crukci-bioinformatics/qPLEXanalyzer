...
pcaPlot <- function(MSnSetObj, omitIgG=FALSE, sampleColours=NULL,
  ...
  ggplot(plotDat, 
         aes_string(x = xPC, y = yPC, fill = colourBy, label = labelColumn)) +
...
maVolPlot <- function(diffstats, contrast, title="", controlGroup = NULL,
  ...
  daResTab <- suppressMessages(getContrastResults(
    ...
    mutate(controlLogFoldChange = 
             ifelse(is.na(controlLogFoldChange), Inf, controlLogFoldChange)) %>%
    mutate(group = 
             ifelse(testSignficant(.), "Significant", "Non-significant")) %>%
  ...
  ggplot(daResTab, aes_string(
    x = xFactor, y = yFactor, colour = "group", size = "group", shape = "group",
    alpha = "group", fill = "group")) +
...
coveragePlot <- function(MSnSetObj, ProteinID, ProteinName, fastaFile, 
  ...
  ## extract peptide sequence from MsnSet object and match peptide sequence with 
  ## protein sequence and store co-ordinates
...
intensityBoxplot <- function(MSnSetObj, title="", sampleColours=NULL, 
  ...
  exprs(MSnSetObj) %>%
    ...
    mutate_at(vars(colourBy), funs(as.factor)) %>%
    ggplot() +
    geom_boxplot(aes_string(x = "SampleName", y = "logInt", fill = colourBy),
                 alpha = 0.6) +
    scale_fill_manual(values = sampleColours, breaks = names(sampleColours)) +
